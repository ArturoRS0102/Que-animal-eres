...
# Guardará los resultados y los bytes de las imágenes mientras el servidor esté activo.
resultados_store = {}
...
def generar_y_obtener_imagen_bytes(animal: str) -> bytes | None:
    """
    Genera una imagen con DALL-E y devuelve su contenido en bytes.
    """
    if not API_KEY:
        print("ADVERTENCIA: No hay API Key. No se puede generar imagen.")
        return None
...
def serve_generated_image(resultado_id):
    resultado = resultados_store.get(resultado_id)
    if not resultado or 'image_content' not in resultado or not resultado['image_content']:
        abort(404)
    return Response(resultado['image_content'], mimetype='image/png')
...
def serve_robots():
    # Servimos el contenido de robots.txt directamente como texto.
    # Esto es mucho más confiable que depender del sistema de archivos.
...
def analizar():
    ...
    try:
        ...
        # Guardar los bytes de la imagen en nuestro almacenamiento en memoria
        resultado_data['image_content'] = image_bytes
        
        # La URL para la etiqueta og:image ahora apunta a nuestra nueva ruta /image/...
        ...
        # Creamos una copia para enviar al cliente, sin los pesados bytes de la imagen.
        response_to_client = resultado_data.copy()
        del response_to_client['image_content']
